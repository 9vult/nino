...
internal static class AzureHelper
{
    ...
    public static Container? Projects { get; private set; }
    public static Container? Episodes { get; private set; }
    public static Container? Configurations { get; private set; }
    public static Container? Observers { get; private set; }
    ...
    public static async Task Setup(string endpointUri, string primaryKey, string databaseName)
    {
        ...
        Projects = await _database.CreateContainerIfNotExistsAsync("Projects", "/guildId");
        Episodes = await _database.CreateContainerIfNotExistsAsync("Episodes", "/projectId");
        Configurations = await _database.CreateContainerIfNotExistsAsync("Configuration", "/guildId");
        Observers = await _database.CreateContainerIfNotExistsAsync("Observers", "/originGuildId");
    }
    ...
    {
        List<T> results = [];
        using var feed = Configurations!.GetItemQueryIterator<T>(queryDefinition: sql);
    ...
    {
        List<T> results = [];
        using var feed = Projects!.GetItemQueryIterator<T>(queryDefinition: sql);
        while (feed.HasMoreResults)
        {
            var response = await feed.ReadNextAsync();
            results.AddRange(response);
    ...
    {
        List<T> results = [];
        using var feed = Episodes!.GetItemQueryIterator<T>(queryDefinition: sql);
        while (feed.HasMoreResults)
        {
            var response = await feed.ReadNextAsync();
            results.AddRange(response);
    ...
    {
        List<T> results = [];
        using var feed = Observers!.GetItemQueryIterator<T>(queryDefinition: sql);
        while (feed.HasMoreResults)
        {
            var response = await feed.ReadNextAsync();
            results.AddRange(response);
