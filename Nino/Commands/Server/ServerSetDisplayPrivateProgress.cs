...
namespace Nino.Commands
{
    public partial class ServerManagement
    {
        ...
        {
            [SlashCommand(
                "publish-private-progress",
                "Control if private project updates should be published"
            )]
            public async Task<RuntimeResult> SetPrivateProgress(bool enabled)
            {
                ...
                config.PublishPrivateProgress = enabled;

                Log.Info(
                    $"Updated configuration for guild {config.GuildId}, set Publish Private Progress to {enabled}"
                );
