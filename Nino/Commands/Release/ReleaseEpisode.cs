...
public partial class Release
{
    ...
    )
    {
        ...
        var roleStr =
            roleId != null
                ? roleId == project.GuildId
                    ? "@everyone "
                    : $"<@&{roleId}> "
                : "";

        var publishTitle = T(
            "progress.release.episode.publish",
            gLng,
            project.Title,
            episodeNumber
        );
        ...
        await ObserverPublisher.PublishRelease(project, publishTitle, url, db);
        ...
        Log.Info(
            $"M[{interaction.User.Id} (@{interaction.User.Username})] released {project} episode {episodeNumber}"
        );
