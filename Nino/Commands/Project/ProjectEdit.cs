...
namespace Nino.Commands
{
    public partial class ProjectManagement
    {
        ...
        )
        {
            ...
            switch (option)
            {
                ...
                case ProjectEditOption.Poster:
                    if (!Uri.TryCreate(newValue, UriKind.Absolute, out _))
                        return await Response.Fail(
                            T("error.project.invalidPosterUrl", lng),
                            interaction
                        );
                ...
                case ProjectEditOption.IsPrivate:
                    ...
                    if (!Bool().IsMatch(input))
                        return await Response.Fail(
                            T("error.incorrectBooleanFormat", lng),
                            interaction
                        );
                ...
                case ProjectEditOption.UpdateChannelID:
                    if (
                        !ulong.TryParse(
                            newValue.Replace("<#", "").Replace(">", "").Trim(),
                            out var updateChannelId
                        )
                ...
                case ProjectEditOption.Nickname:
                    ...
                    if (db.Projects.Any(p => p.GuildId == guildId && p.Nickname == newValue))
                        return await Response.Fail(
                            T("error.project.nameInUse", lng, newValue),
                            interaction
                        );
            ...
            var embedDescription = T(
                "project.edited",
                lng,
                project.Nickname,
                option.ToFriendlyString(lng)
            );
