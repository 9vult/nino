...
internal static class Utils
{
    ...
    public static bool VerifyAdministrator(SocketGuildUser member, SocketGuild guild, bool excludeServerAdmins = false)
    {
        ...
        return Cache.GetConfig(guild.Id)?.AdministratorIds.Any(a => a == member.Id) ?? false;
    }
    ...
    public static bool VerifyUser(ulong userId, Project project, bool excludeAdmins = false, bool includeKeyStaff = false)
    {
        ...
        if (excludeAdmins) return includeKeyStaff && project.KeyStaff.Any(ks => ks.UserId == userId);
        ...
        return includeKeyStaff && project.KeyStaff.Any(ks => ks.UserId == userId);
    }
    ...
    public static bool VerifyTaskUser(ulong userId, Project project, Episode episode, string abbreviation)
    {
        ...
        if (Cache.GetConfig(project.GuildId)?.AdministratorIds?.Any(a => a == userId) ?? false)
            return true;
        return episode.PinchHitters.Any(ph => ph.Abbreviation == abbreviation && ph.UserId == userId) 
               || project.KeyStaff.Concat(episode.AdditionalStaff).Any(ks => ks.Role.Abbreviation == abbreviation && ks.UserId == userId);
    }
