...
using Nino.Core.Features.Queries.Observers.ListNamesForGroup;
using Nino.Discord.Services;

namespace Nino.Discord.Handlers.AutocompleteHandlers;

public sealed class GroupObserverAutocompleteHandler : AutocompleteHandler
{
    /// <inheritdoc />
    ...
    )
    {
        ...
        var handler = scope.ServiceProvider.GetRequiredService<ListObserverNamesForGroupHandler>();
        ...
        var result = await handler.HandleAsync(
            new ListObserverNamesForGroupQuery(groupId, userId, isDiscordAdmin)
        );
