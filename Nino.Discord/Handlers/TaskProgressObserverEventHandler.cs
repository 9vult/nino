...
using Nino.Core.Features.Queries.Observers.GetUpdateNotificationData;
...
public sealed class TaskProgressObserverEventHandler(
    DiscordSocketClient client,
    IBotPermissionsService botPermissionsService,
    GetObserverUpdateNotificationDataHandler getDataHandler,
    GetProgressResponseDataHandler getProgressResponseDataHandler,
    ILogger<TaskProgressObserverEventHandler> logger
) : IEventHandler<TaskProgressObserverEvent>
{
    /// <inheritdoc />
    public async Task HandleAsync(TaskProgressObserverEvent @event)
    {
        var (observerId, _, episodeId, taskId, progressType) = @event;
        ...
        {
            logger.LogWarning("Failed to get progress data for observer {ObserverId}", observerId);
            return;
        }
        ...
        logger.LogInformation(
            "Publishing task progress ({ProgressType}) for {ProjectTitle} {EpisodeNumber} {Abbreviation} to observer channel {Channel}",
            progressType,
