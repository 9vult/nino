...
public sealed class BatchReleasedEventHandler(
    DiscordSocketClient client,
    IBotPermissionsService botPermissionsService,
    GetReleaseNotificationDataHandler getDataHandler,
    ILogger<BatchReleasedEventHandler> logger
) : IEventHandler<BatchReleasedEvent>
{
    /// <inheritdoc />
    public async Task HandleAsync(BatchReleasedEvent @event)
    {
        var (
            projectId,
            firstNumber,
        ...
        logger.LogInformation(
            "Publishing batch release for {ProjectTitle} {FirstNumber}-{LastNumber} to {Channel}",
            data.ProjectTitle,
            firstNumber,
            lastNumber,
            channel
