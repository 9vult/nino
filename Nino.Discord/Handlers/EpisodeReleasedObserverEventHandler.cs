...
public sealed class EpisodeReleasedObserverEventHandler(
    DiscordSocketClient client,
    IBotPermissionsService botPermissionsService,
    GetObserverReleaseNotificationDataHandler getDataHandler,
    ILogger<EpisodeReleasedObserverEventHandler> logger
) : IEventHandler<EpisodeReleasedObserverEvent>
{
    ...
    public async Task HandleAsync(EpisodeReleasedObserverEvent @event)
    {
        ...
        logger.LogInformation(
            "Publishing episode release for {ProjectTitle} {EpisodeNumber} to observer channel {Channel}",
            data.ProjectTitle,
            episodeNumber,
            channel
        );
