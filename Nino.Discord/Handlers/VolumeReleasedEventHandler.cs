...
public sealed class VolumeReleasedEventHandler(
    DiscordSocketClient client,
    IBotPermissionsService botPermissionsService,
    GetReleaseNotificationDataHandler getDataHandler,
    ILogger<VolumeReleasedEventHandler> logger
) : IEventHandler<VolumeReleasedEvent>
{
    /// <inheritdoc />
    public async Task HandleAsync(VolumeReleasedEvent @event)
    {
        var (
            projectId,
            volumeNumber,
        ...
        logger.LogInformation(
            "Publishing volume release for {ProjectTitle} {EpisodeNumber} to {Channel}",
            data.ProjectTitle,
            volumeNumber,
            channel
        );
