...
public sealed class CustomReleasedObserverEventHandler(
    DiscordSocketClient client,
    IBotPermissionsService botPermissionsService,
    GetObserverReleaseNotificationDataHandler getDataHandler,
    ILogger<CustomReleasedObserverEventHandler> logger
) : IEventHandler<CustomReleasedObserverEvent>
{
    /// <inheritdoc />
    public async Task HandleAsync(CustomReleasedObserverEvent @event)
    {
        var (_, observerId, label, urls, commentary, publish) = @event;
        ...
        logger.LogInformation(
            "Publishing episode release for {ProjectTitle} \"{ReleaseLabel}\" to observer channel {Channel}",
            data.ProjectTitle,
            label ?? string.Empty,
            channel
        );
