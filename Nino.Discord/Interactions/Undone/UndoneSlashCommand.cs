...
namespace Nino.Discord.Interactions.Undone;
...
public class UndoneSlashCommand(
    ...
    MarkTaskUndoneHandler undoneHandler
) : InteractionModuleBase<IInteractionContext>
{
    [SlashCommand("undone", "Mark a task as not done")]
    public async Task<RuntimeResult> HandleUndoneAsync(
    ...
    )
    {
        ...
        var result = await undoneHandler
            .HandleAsync(new MarkTaskUndoneCommand(projectId, episodeId, taskId, requestedBy))
        ...
        var taskInfo = result.Value.Item1;
        var progressResponseInfo = result.Value.Item2;
        var pData = result.Value.Item3;

        var body = new StringBuilder();
        body.AppendLine(T("undone.success", locale, taskInfo.EpisodeNumber, taskInfo.TaskName));
        ...
        var embed = new EmbedBuilder()
            ...
            .WithTitle($"❌ {T("undone.response.title", locale)}")
