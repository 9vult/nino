...
{
    [ComponentInteraction("nino.release.custom.confirm:*", ignoreGroupNames: true)]
    public async Task<RuntimeResult> ConfirmCustomReleaseAsync(string rawId)
    {
        ...
        var command = await stateService.LoadStateAsync<ReleaseCustomCommand>(stateId);
        if (command is null)
        ...
        var result = await releaseCustomHandler
            .HandleAsync(command)
            .BindAsync(() =>
        ...
        var successEmbed = new EmbedBuilder()
            ...
            .WithDescription(T("release.custom.success", locale))
            .Build();
