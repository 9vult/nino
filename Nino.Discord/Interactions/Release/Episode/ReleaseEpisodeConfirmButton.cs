...
{
    [ComponentInteraction("nino.release.episode.confirm:*", ignoreGroupNames: true)]
    public async Task<RuntimeResult> ConfirmEpisodeReleaseAsync(string rawId)
    {
        ...
        var command = await stateService.LoadStateAsync<ReleaseEpisodeCommand>(stateId);
        if (command is null)
        ...
        var result = await releaseEpisodeHandler
            .HandleAsync(command)
            .BindAsync(() =>
        ...
        var successEmbed = new EmbedBuilder()
            ...
            .WithDescription(T("release.episode.success", locale, command.Number))
            .Build();
