...
public sealed class ReleaseEpisodeHandler(
    NinoDbContext db,
    IUserVerificationService verificationService,
    IEventBus eventBus,
    ILogger<ReleaseEpisodeHandler> logger
) : ICommandHandler<ReleaseEpisodeCommand, Result>
{
    /// <inheritdoc/>
    public async Task<Result> HandleAsync(ReleaseEpisodeCommand command)
    {
        ...
        logger.LogInformation(
            "Publishing Release of project {ProjectId} Episode {ReleaseEpisode} to local group and {ObserverCount} observers",
            command.ProjectId,
            command.Number,
        ...
        [
            ...
            .. project.Observers.Select(observer =>
                eventBus.PublishAsync(
                    new EpisodeReleasedObserverEvent(
                        ProjectId: command.ProjectId,
