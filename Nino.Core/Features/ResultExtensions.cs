...
{
    /// <summary>
    /// Awaits <paramref name="resultTask"/> and, if successful, invokes
    ...
    /// <param name="next">The async operation to invoke on success.</param>
    /// <returns>
    /// The result of <paramref name="next"/> if the incoming result succeeded;
    /// otherwise, a failed <see cref="Result"/> carrying the original status and message.
    /// </returns>
    ...
    )
    {
        ...
        return result.IsSuccess ? await next() : Result.Fail(result.Status, result.Message);
    }

    /// <summary>
    /// If <paramref name="result"/> is successful, invokes <paramref name="next"/>.
    ...
    {
        var result = await resultTask;
        return result.IsSuccess ? await next() : Result<TOut>.Fail(result.Status, result.Message);
    }
