...
public class MarkTaskUndoneHandlerTests : TestBase
{
    [Test]
    public async Task Assignee_Command_Marks_Task_Undone_And_Publishes_Notifications()
    {
        ...
        var handler = new MarkTaskUndoneHandler(
            db,
            UserVerificationService,
            EventBus,
            NullLogger<MarkTaskUndoneHandler>.Instance
        );

        var command = new MarkTaskUndoneCommand(
            seed.ProjectId,
            seed.Episode1Id,
            seed.Task2Id1,
            seed.User2Id
        ...
        await Assert.That(task.IsDone).IsFalse();
    ...
    [Test]
    public async Task Admin_Command_Marks_Task_Undone_And_Publishes_Notifications()
    {
        var seed = await Db.SeedAsync(IdentityService);
        var db = Db.Context;
    ...
    [Test]
    public async Task Incomplete_Task_ReturnsBadRequest()
    {
        var seed = await Db.SeedAsync(IdentityService);
        var db = Db.Context;
