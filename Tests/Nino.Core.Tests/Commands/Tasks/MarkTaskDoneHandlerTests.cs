...
public class MarkTaskDoneHandlerTests : TestBase
{
    [Test]
    public async Task Assignee_Command_Marks_Task_Done_And_Publishes_Notifications()
    {
        ...
        var handler = new MarkTaskDoneHandler(
            db,
            UserVerificationService,
            EventBus,
            NullLogger<MarkTaskDoneHandler>.Instance
        );

        var command = new MarkTaskDoneCommand(
            seed.ProjectId,
            seed.Episode2Id,
            seed.Task2Id2,
            seed.User2Id
